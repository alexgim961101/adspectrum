...
광고 성과 이벤트를 수집·집계하는 파이프라인을 EKS 위에 IaC와 GitOps로 구축·운영하는 개인 프로젝트다. **약 1주 안에 완성**하는 것을 전제로 범위를 잡았다.

목적은 두 가지를 하나의 프로젝트로 증명하는 것이다.

1. **기술 폭**: EKS, Terraform, GitHub Actions, ArgoCD, Argo Rollouts, Helm, KEDA, Prometheus/Grafana를 실제로 구축·운영할 수 있다.
2. **도메인**: 광고 성과 데이터를 실시간으로 수집·적재하는 시스템이 하는 일을 축소판으로 직접 구현한다.

프로젝트 이름 `adspectrum`은 프리즘을 통과한 빛이 스펙트럼이 되듯, 파편화된 광고 이벤트 스트림을 분해·집계해 보이게 만든다는 의미다.
...
| EDA(이벤트 드리븐), 대규모 트래픽/데이터 파이프라인 | SQS 기반 이벤트 파이프라인 + KEDA 큐 길이 오토스케일링 |
| Argo Rollout | metrics-api 카나리 배포 + Prometheus 분석 기반 자동 롤백 |
| AI 도구를 쓰는 개발 방식 | 전 과정을 Claude Code로 개발, 레포 CLAUDE.md와 README에 활용 방식 기록 |
...
**의사결정 — 모노레포**: GitOps 정석은 앱 레포와 배포 레포 분리지만, 1인 프로젝트에서는 읽는 사람이 링크 하나로 전체를 파악하는 가치가 더 크다. 분리 원칙은 `deploy/` 경로 격리로 표현하고, CI 루프는 아래 두 장치로 방지한다 (README에 트레이드오프 기록).
...
- 상태 파일: S3 백엔드 + DynamoDB 잠금. 처음에는 로컬 state로 시작했고, CI가 PR마다 `plan`을 돌리려면 상태를 읽어야 해서 옮겼다 (DECISIONS 015). 버킷·잠금 테이블·CI 역할은 환경과 함께 지워지면 안 되므로 계정 사전 조건으로 둔다. (스테이트 파일과 `.terraform/`은 절대 커밋하지 않는다.)
- 모듈 구성:
...
- [x] 아키텍처 다이어그램 (mermaid)
- [x] 역량 맵 (1장 표 재사용)
- [x] ArgoCD 앱 트리 Synced/Healthy 스크린샷
- [x] KEDA 스케일링 그래프 (큐 깊이 + 레플리카 수 겹친 패널)
- [x] 카나리 자동 롤백 진행 화면 캡처
- [x] CI 성공 런 링크
- [x] 설계 의사결정 기록 (모노레포, SQS, 중복 허용, 단일 NAT, 상태 백엔드, 비용 통제)
- [x] 확장 로드맵
- [x] AI 도구를 쓴 개발 방식 섹션 (Claude Code 활용 방식: 레포 CLAUDE.md, 스펙 기반 구현 흐름)
- [x] 실행 방법 (`terraform apply`부터 데모 재현까지 명령 순서)
