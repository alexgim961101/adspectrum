...
2026-08-29 Karpenter·External Secrets·FIS까지 더한 상태에서 다시 확인했다 —
**101개 삭제, 11분 26초**, `state list` 0줄. 리소스가 27개 늘었는데 시간은 거의 같다.
대부분이 IAM·SQS·EventBridge라 삭제가 빠르고, 시간을 지배하는 것은 여전히 EKS
컨트롤플레인과 NAT Gateway다.

**의도적으로 남는 것이 늘었다.** 상태 버킷, 잠금 테이블, `adspectrum-ci-plan` 역할,
SSM의 알림 웹훅은 환경보다 오래 살아야 하므로 Terraform이 만들지 않는다
(DECISIONS 015, 017). destroy 뒤 고아 리소스를 검사할 때 이 넷은 정상이다.
