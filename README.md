...
![ArgoCD app-of-apps 트리](docs/images/argocd-app-tree.png)

뿌리 Application 하나가 나머지 열두 개를 관리합니다. 사람이 손으로 넣는 것은 이 뿌리
...
| 감지에서 통보까지 잇는다 | 지표는 모으는데 통보 경로가 없어 사람이 대시보드를 봐야 했습니다 | 스키마 위반 메시지 주입 → 규칙 발화 → Slack 전송 6건·실패 0 |
| 비밀을 클러스터 밖으로 | 알림 웹훅이 운영자 노트북에만 있었습니다 | 새 클러스터에서 사람 개입 없이 `SecretSynced` |
| 노드도 자동으로 늘린다 | KEDA가 파드를 늘려도 노드는 고정이라 한 계층에서 끊겼습니다 | 스케줄 불가 파드 → **33초** 만에 노드 생성, 유휴 시 자동 삭제 |
| 위험을 실험으로 확인한다 | spot 중단 대응이 문서로만 있었습니다 | FIS로 회수를 일으켜 **1분 만에** 복구 확인 |
| 취약점을 푸시 전에 막는다 | ECR 스캔은 켜져 있었지만 아무도 결과를 보지 않았습니다 | 붙인 첫 실행에서 openssl CVE 적발 |

마지막 줄은 붙이자마자 값을 했습니다. 검사를 켠 첫 실행에서 이렇게 걸렸습니다.

```
libssl3t64 / openssl / openssl-provider-legacy
CVE-2026-14456 (HIGH) · OpenSSL DoS
설치됨 3.5.6-1~deb13u2  →  수정본 3.5.7-1~deb13u2
```

수정본은 데비안 저장소에 이미 있는데 `python:3.12-slim` 베이스가 아직 그것을 담아
다시 구워지지 않은 상태였습니다. 베이스의 재빌드 주기는 우리가 통제할 수 없으므로,
런타임 단계에서 OS 패치를 직접 받도록 고쳤습니다. **ECR의 `scan_on_push`는 그 전에도
켜져 있었지만** 아무도 결과를 보지 않았기 때문에 똑같이 몰랐습니다 — 켜 두는 것과
막는 것은 다릅니다.

**확인 전에는 동작한다고 적지 않습니다.** 위 네 줄은 전부 클러스터를 올려 실제로
돌려 본 결과입니다. 이 원칙 덕분에 카나리 분석이 조용히 실패한 것을 잡아냈습니다
(아래 카나리 절).
...
![Slack에 도착한 알림](docs/images/slack-alert-delivery.png)

규칙이 발화해 Slack까지 도착합니다. 위쪽은 처음에 잘못 나갔던 메시지입니다 — 차트가
`tpl`을 돌리지 않는 값에 Helm 이스케이프를 씌워서 템플릿 문자열이 그대로 나갔습니다.
아래가 고친 뒤입니다 ([DECISIONS 016](docs/DECISIONS.md)).
...
스케일링은 두 계층입니다. KEDA가 큐 길이로 파드를 늘리고, 파드가 들어갈 자리가 없으면
Karpenter가 노드를 만듭니다. 다만 **이 워크로드는 노드를 늘릴 만큼 무겁지 않습니다** —
초당 900건까지 올려 컨슈머가 상한 10까지 늘어나도 요청이 50m이라 기존 노드 2대에 다
들어갑니다. 그래서 Karpenter는 들어갈 수 없는 파드를 따로 만들어 확인했습니다
([DECISIONS 018](docs/DECISIONS.md)).
...
- **레지스트리는 환경 밖에 둡니다** — ECR과 CI 푸시 역할을 클러스터와 같은 스택에 두면
  환경이 내려간 동안 CI가 실패합니다. 실제로 겪고 옮겼습니다. 덤으로 이미지가 destroy를
  넘어 살아남아, 재기동 때 이미지를 다시 굽는 절차가 없어졌습니다.
